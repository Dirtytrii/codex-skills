...
`UI/PPT` loads `$ui-implementation-workflow`. Classify one page type, audit the existing system, use at most three role-specific references, extract an implementation plan before code, reuse semantic tokens, build skeleton-first, and close with repaired screenshots at 1440/768/390. Operational dashboards/forms do not inherit marketing-page heroes or decorative motion.
