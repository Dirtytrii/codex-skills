---
name: gstack-spec
description: GStack spec method for turning fuzzy intent into an executable spec with goals, non-goals, acceptance criteria, risks, and downstream role boundaries.
---

# GStack Spec Adapter

This is a Codex role-system adapter for Garry Tan's gstack `gstack-spec` method.

## When To Use

Use this when:

- the user has intent but no implementation-ready shape;
- a downstream developer prompt needs a clearer spec;
- acceptance criteria or non-goals are missing;
- the user explicitly invokes `$gstack-spec`.
...
4. Produce: a concise executable spec with goals, non-goals, scope, acceptance criteria, risks, and open questions.
