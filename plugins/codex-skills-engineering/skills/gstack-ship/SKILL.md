---
name: gstack-ship
description: GStack ship method for final landing, PR, changelog, release, and deploy readiness checklists when the user explicitly wants to ship.
---

# GStack Ship Adapter

This is a Codex role-system adapter for Garry Tan's gstack `gstack-ship` method.

## When To Use

Use this when:

- the user explicitly asks to ship, land, release, or prepare a PR;
- work is near final acceptance;
- release notes and validation gates are needed;
- the user explicitly invokes `$gstack-ship`.
...
4. Produce: ship checklist, validation results, changelog/PR notes, rollback or no-ship reasons.
...
- Do not deploy, push, merge, or mutate production unless the active task explicitly authorizes it.
