...
- `UI/PPT`: 所有实质 UI/Frontend 工作先用 `ui-implementation-workflow` 做页面分类、完整参考源库存、激活参考预算、换源台账、规则提取、Token、骨架优先和三视口截图闭环；失败时只切换对应职责位，禁止无限浏览或一次加载所有站点；营销页/作品集再按需用 `design-taste-frontend`，其他视觉资产按任务使用 `guizang-ppt-skill`、`xhs-visual-director`、`guizang-social-card-skill`、`photo-to-cute-3d-toy`；浏览器验证用 `browser-automation-router`，CI/回归按需用 `playwright`
