...
- `UI/PPT`: 所有实质 UI/Frontend 工作先用 `ui-implementation-workflow` 做页面分类、参考预算、规则提取、Token、骨架优先和三视口截图闭环；营销页/作品集再按需用 `design-taste-frontend`，其他视觉资产按任务使用 `guizang-ppt-skill`、`xhs-visual-director`、`guizang-social-card-skill`、`photo-to-cute-3d-toy`；浏览器验证用 `browser-automation-router`，CI/回归按需用 `playwright`
