...
- `UI/PPT`: 所有实质 UI/Frontend 工作统一使用 `ui-implementation-workflow`；营销、品牌、作品集或内容页在现有系统不足时按需读取其 `references/visual-direction.md`，后台/表单/列表/配置页不加载。`design-taste-frontend` 仅保留为旧名称兼容入口，不再运行独立审计、实现或截图流程。旧审美偏好不再参与决策；新工作流只把用户或指定审核者对实际截图的明确反馈记录到项目级 `.codex/ui-visual-review-signals.md`，并保持为 raw 证据。主 workflow 继续负责参考预算、换源台账、Token、骨架优先和三视口截图闭环；其他视觉资产按任务使用 `guizang-ppt-skill`、`xhs-visual-director`、`guizang-social-card-skill`、`photo-to-cute-3d-toy`；浏览器验证用 `browser-automation-router`，CI/回归按需用 `playwright`
