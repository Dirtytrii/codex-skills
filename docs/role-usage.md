...
这份文档说明本仓库的结构、角色边界和推荐运行环境。核心原则是：`架构` 先判断需求、为新本地代码项目启动 CodeGraph、扫描开源/可借鉴方案并拆分边界，复杂需求先给多方案技术选型，`开发` 等执行角色按白名单落地，`公众号发布` / `小红书` / `文档/交付` / `知识库` / `DBA` 可作为独立角色单独唤起，`运维` 优先交给服务器侧 Hermes agent 做只读诊断和受控操作。
...
6. `架构` 只在必要时输出 `开发`、`UI/PPT` / `UI/Frontend`、`视频`、`公众号发布`、`小红书`、`文档/交付`、`知识库`、`DBA`、`测试`、`QA`、`安全`、`运维` 等下游提示词。
7. 已建立过的角色默认走 `继承` / `接续`，不要重复新建窗口。
8. 下游角色完成、阻塞或需要发起方决策时，默认回调任务发起窗口；不无条件回报给 `架构`。
